...
type RoomItem = {
  ...
  host:  { id: string; name: string | null; image: string | null; battleshipElo: number };
  guest?: { id: string; name: string | null; image: string | null; battleshipElo: number } | null;
};
...
export default function BattleshipOnlineLobby() {
  ...
  async function fetchRooms() {
    const res = await fetch("/api/battleship-rooms");
  ...
  async function createRoom() {
    setCreating(true);
    try {
      const res = await fetch("/api/battleship-rooms", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ timeControl }) });
      if (res.ok) { const { id } = await res.json(); router.push(`/games/battleship/online/${id}`); }
  ...
  async function joinRoom(roomId: string) {
    ...
    try {
      ...
      if (res.ok) router.push(`/games/battleship/online/${roomId}`);
          ...
          <div className="flex flex-col gap-3">
            {playing.map(room => (
              <div key={room.id} className="flex items-center gap-4 bg-[var(--bg-elevated)] border border-[var(--border-subtle)] rounded-xl px-4 py-3">
                ...
                <Link href={`/games/battleship/online/${room.id}`}
