...
export default function BilliardsOnlineLobby() {
  ...
  async function fetchRooms() {
    const res = await fetch("/api/billiards-rooms");
    if (res.ok) {
  ...
  async function createRoom() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        router.push(`/games/billiards/online/${id}`);
      } else {
  ...
  async function joinRoom(roomId: string) {
    ...
    try {
      ...
      if (res.ok) router.push(`/games/billiards/online/${roomId}`);
  ...
  return (
    <main className="max-w-2xl mx-auto px-4 py-12">
      ...
      <h1 className="text-3xl font-display font-extrabold text-[var(--text-primary)] mb-1">🎱 Online Billiards</h1>
      <p className="text-[var(--text-muted)] mb-8">
        1 vs 1 in real-time ·{" "}
        <Link href="/games/billiards/online/rated" className="text-pink-400 hover:opacity-80">Rated queue →</Link>
      </p>
          ...
          {waiting.map(room => (
            <div key={room.id} className="flex items-center gap-4 bg-[var(--bg-elevated)] border border-[var(--border-subtle)] rounded-xl px-4 py-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-[var(--text-muted)] text-xs">{TC_LABELS[room.timeControl] ?? room.timeControl} · ELO {room.host.billiardsElo}</p>
              </div>
