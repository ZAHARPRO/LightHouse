...
type RoomItem = {
  ...
  host:  { id: string; name: string | null; image: string | null; billiardsElo: number };
  guest?: { id: string; name: string | null; image: string | null; billiardsElo: number } | null;
};
...
export default function RatedBilliardsLobby() {
  ...
  const queue = useMatchmakingQueue({
    ...
    startSearch: async () => {
      const res = await fetch("/api/billiards-rooms/matchmake", {
        method: "POST",
      ...
      if (matched) { playSound("opponent_found"); router.push(`/games/billiards/online/${roomId}`); return null; }
      return roomId;
    },
    cancelSearch: async () => { await fetch("/api/billiards-rooms/matchmake", { method: "DELETE" }); },
    checkMatch: async (roomId) => {
      const r = await fetch(`/api/billiards-rooms/${roomId}`);
      if (!r.ok) return null;
      const room = await r.json();
      if (room.guestId) { playSound("opponent_found"); return `/games/billiards/online/${roomId}`; }
      return null;
    },
      ...
      {playing.length > 0 && (
        <>
          ...
          <div className="flex flex-col gap-3">
            {playing.map(room => {
              const hostRank  = getRank(room.host.billiardsElo);
              const guestRank = room.guest ? getRank(room.guest.billiardsElo) : null;
              return (
                <div key={room.id} className="flex items-center gap-4 bg-[var(--bg-elevated)] border border-[var(--border-subtle)] rounded-xl px-4 py-3">
                  ...
                  <Link href={`/games/billiards/online/${room.id}`}
