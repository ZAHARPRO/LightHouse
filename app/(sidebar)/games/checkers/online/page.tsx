...
type RoomItem = {
  id: string; timeControl: string; guestId: string | null; createdAt: string;
  spectatorCount: number; rated: boolean;
  host:  { id: string; name: string | null; image: string | null; checkersElo: number };
  guest?: { id: string; name: string | null; image: string | null; checkersElo: number } | null;
};
...
export default function CheckersOnlineLobby() {
  ...
  async function fetchRooms() {
    const res = await fetch("/api/checkers-rooms");
    if (res.ok) { const d = await res.json(); setWaiting(d.waiting ?? []); setPlaying(d.playing ?? []); }
  }

  useEffect(() => { fetchRooms(); const t = setInterval(fetchRooms, 3000); return () => clearInterval(t); }, []);

  async function createRoom() {
    setCreating(true);
    try {
      const res = await fetch("/api/checkers-rooms", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ timeControl }) });
      if (res.ok) { const { id } = await res.json(); router.push(`/games/checkers/online/${id}`); }
  ...
  async function joinRoom(roomId: string) {
    ...
    try {
      ...
      if (res.ok) router.push(`/games/checkers/online/${roomId}`);
    ...
    <main className="max-w-2xl mx-auto px-4 py-12">
      ...
      <div className="bg-[var(--bg-elevated)] border border-[var(--border-subtle)] rounded-2xl p-5 mb-6">
        ...
        <button onClick={createRoom} disabled={creating}
          ...
          {creating ? <Loader2 size={15} className="animate-spin" /> : <Plus size={15} />} Create Room
        </button>
      </div>
          ...
          {waiting.map(room => (
            <div key={room.id} className="flex items-center gap-4 bg-[var(--bg-elevated)] border border-[var(--border-subtle)] rounded-xl px-4 py-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-[var(--text-muted)] text-xs">{TC_LABELS[room.timeControl] ?? room.timeControl} · ELO {room.host.checkersElo}</p>
              </div>
              <button onClick={() => joinRoom(room.id)} disabled={joiningId === room.id}
                ...
                {joiningId === room.id && <Loader2 size={12} className="animate-spin" />} Join
              </button>
            </div>
          ))}
        </div>
      ...
      {playing.length > 0 && (
        <>
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-[var(--text-muted)] text-sm font-display font-semibold">Live Games ({playing.length})</span>
          </div>
            ...
            {playing.map(room => (
              <div key={room.id} className="flex items-center gap-4 bg-[var(--bg-elevated)] border border-[var(--border-subtle)] rounded-xl px-4 py-3">
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-display font-semibold text-[var(--text-primary)] text-sm truncate">{room.host.name ?? "?"} vs {room.guest?.name ?? "?"}</p>
                  <p className="text-[var(--text-muted)] text-xs">{TC_LABELS[room.timeControl] ?? room.timeControl}</p>
                </div>
                <Link href={`/games/checkers/online/${room.id}`}
                  ...
                  <Eye size={12} /> Watch {room.spectatorCount > 0 && <span className="ml-0.5 text-[0.6rem] opacity-80">{room.spectatorCount}</span>}
                </Link>
              </div>
            ))}
          </div>
