...
export default function CheckersRatedPage() {
  ...
  const queue = useMatchmakingQueue({
    ...
    startSearch: async () => {
      ...
      if (matched) { router.push(`/games/checkers/online/${roomId}`); return null; }
      return roomId;
    },
    cancelSearch: async () => { await fetch("/api/checkers-rooms/matchmake", { method: "DELETE" }).catch(() => {}); },
    checkMatch: async (roomId) => {
      const r = await fetch(`/api/checkers-rooms/${roomId}`);
      if (!r.ok) return null;
      const room = await r.json();
      if (room.guestId) return `/games/checkers/online/${roomId}`;
      if (room.status === "FINISHED") return null; // cancelled — stay idle, cancel handles redirect
      return null;
    },
