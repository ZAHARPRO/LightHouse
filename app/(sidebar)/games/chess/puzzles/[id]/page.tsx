...
function VideoModal({ onClose, onRecharged }: { onClose:()=>void; onRecharged:(pts:number)=>void }) {
  ...
  // Timer only ticks when video is actually playing
  useEffect(() => {
  ...
  // Auto-claim when watched (only if user hasn't closed the modal)
  useEffect(() => {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[960] flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
      <div className="bg-[var(--bg-elevated)] border border-[var(--border-subtle)] rounded-2xl p-6 w-full max-w-md relative">
        {/* Close button only shown for non-YouTube videos (YouTube close is in the player overlay) */}
        {video && !getYouTubeId(video.url) && (
          <button onClick={handleClose} className="absolute top-4 right-4 text-[var(--text-muted)] hover:text-[var(--text-primary)]"><X size={18}/></button>
        )}
        {!video && !loading && (
          <button onClick={handleClose} className="absolute top-4 right-4 text-[var(--text-muted)] hover:text-[var(--text-primary)]"><X size={18}/></button>
        )}
