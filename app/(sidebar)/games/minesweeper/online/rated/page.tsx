...
export default function RatedMinesweeperLobby() {
  ...
  const queue = useMatchmakingQueue({
    ...
    startSearch: async () => {
      const res = await fetch("/api/ms-rooms/matchmake", {
        method: "POST",
      ...
      if (matched) { playSound("opponent_found"); router.push(`/games/minesweeper/online/${roomId}`); return null; }
      return roomId;
    },
    cancelSearch: async () => { await fetch("/api/ms-rooms/matchmake", { method: "DELETE" }); },
    checkMatch: async (roomId) => {
      const r = await fetch(`/api/ms-rooms/${roomId}`);
      if (!r.ok) return null;
      const room = await r.json();
      if (room.guestId) { playSound("opponent_found"); return `/games/minesweeper/online/${roomId}`; }
      return null;
    },
      ...
      >
        ...
        <div className="flex gap-2 mb-5">
          {DIFF_OPTIONS.map(opt => (
            <button key={opt.value} onClick={() => setDifficulty(opt.value)} disabled={queue.searching}
              className={pill(difficulty === opt.value)}>
              {opt.icon} {opt.label}
            </button>
          ))}
        </div>
