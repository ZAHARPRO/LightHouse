...
export async function POST(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const room = await prisma.battleshipRoom.findUnique({ where: { id }, select: { spectatorsJson: true, hostId: true, guestId: true } });
  ...
  await prisma.battleshipRoom.update({ where: { id }, data: { spectatorsJson: JSON.stringify(filtered) } });
