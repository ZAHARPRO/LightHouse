...
import { toggleStreamReaction } from "@/actions/streamReactions";
...
export default function StreamReactionButtons({
  ...
  function handleClick(type: "LIKE" | "DISLIKE") {
    if (isOwner || !isLoggedIn || pending) return;
    ...
    startTransition(async () => {
      const res = await toggleStreamReaction(streamId, type);
      if ("error" in res) {
        setReaction(prev);
  ...
  const title = isOwner ? "You own this stream" : !isLoggedIn ? "Sign in to react" : undefined;
